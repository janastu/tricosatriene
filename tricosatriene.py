...
class TricoServer(object):
    ...
    def __init__(self, database="mango", host='localhost', port=27017,
        ...
        self.server_identity = {"type": "Software", "label": "Tricosatriene v0.1", "homepage": "https://github.com/janastu/tricosatriene/"}
    ...
    def _handle_ld_json(self):
        ...
        self.app.logger.debug("get json")
        if request.is_json:
            b = request.get_json(force=True, silent=True)
        else:
            b = ''
        self.app.logger.debug("after request.json")
        if request.headers.get('Content-Type', '').strip().startswith("application/ld+json"):
            ...
            if b:
                if type(b) == bytes:
                    request._json = json.loads(b.decode('utf-8'))
                elif type(b) == dict:
                    request._json = b
                else:
    ...
    def _conneg(self, data, uri):
        ...
        response.headers['ETag'] = h.hexdigest()
    ...
    def get_container_page(self, container, coll, metadata):
        ...
        include = request.args.get('include', self.server_prefers)
    ...
    def dispatch_views(self):
        ...
        for m in methods:
            self.app.add_url_rule('/%s<string:container>/' % self.url_prefix,
                view_func=getattr(self, "%s_container" % m, self.not_implemented), methods=[m.upper()])
            self.app.add_url_rule('/%s<string:container>/<string:resource>' % self.url_prefix,
                view_func=getattr(self, "%s_resource" % m, self.not_implemented), methods=[m.upper()])
    ...
    def error(self, error, message=None):
        ...
        return self._jsonify({"error": error.code,
                        "message": error.get_body() or message}, "")
    ...
    def get_flask_app(self):
        self.app = Flask("TricoServer")
        self.dispatch_views()
        self.app.before_request(self.before_request)
        self.app.after_request(self.after_request)
        for k,v in self.get_error_handler().items():
            self.app.register_error_handler(k, v)
        return self.app
...
def main():
    ...
    run(host=host, port=port, app=mr.get_flask_app(), debug=True)
